...
function sendMail($email, $admin_role, $adminUsername, $password)
{
    ...
    $senderFrom = "Dopemind Studios";
    $subject = "Admin at Dopemind Studios";

    $body = file_get_contents("../emailtoadmin.html");
    $body = str_replace(["(Heading)","(SubHeading)","{{username}}","{{password}}","{{role}}","{Footer Info}"], ["You were chosen as an admin at $senderFrom","Here is your login details",$adminUsername,$password,$admin_role,"Your can change the username and password later,please keep your password secretly"], $body);
