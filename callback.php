...
use PHPMailer\PHPMailer\Exception;
    ...
    function saveSold($itemname,$user,$amount,$itemtype,$itemLeaseType,$conn,$reference,$ids,$images,$files){
      ...
      $query = "INSERT INTO `view_sold`( `item_id`,`item_name`, `sold_to`, `amount_sold`, `date_sold`, `item_type`,`item_lease_type`,`reference_id`,`files`) VALUES ('$ids','$itemname','$user','$amount','$date','$itemtype','$itemLeaseType','$reference','$files')";
      $result = mysqli_query($conn,$query);
      if($result){
    ...
    function sendEmail($email,$ids,$conn,$reference){
      $dom = new DOMDocument();
      $dom -> loadHTMLFile("./emailtouser.html");
      $a = $dom->getElementById("link");
      $website ="https://alabah.com/download.php?reference=$reference";
      ...
      //Recipients
      $mail->setFrom('nsudekenechi2@gmail.com', "$senderFrom");
      //Add a recipient
    ...
    if(isset($_GET["deletePremium"])){
      $details = json_decode($_GET["deletePremium"]);
      $type = $details["type"];
      $id = $details["id"];
      ...
      if($type=="beat"){
        ...
        $result = mysqli_query($conn,$query);
        $row = mysqli_fetch_assoc($result);
        $files = [$row["beat_basic_file"],$row["beat_exclusive_file"],$row["beat_premium_file"],$row["beat_image"]];

        // Deleting Every File For Sold Beat
        foreach($file as $files){
          unlink("./admin/Files/beat/$file");
        }

        $query = "DELETE FROM add_beat WHERE beat_id='$id'";
        $result = mysqli_query($conn,$query);
      }else if($type=="lyrics"){
        ...
        $result = mysqli_query($conn,$query);
        $row = mysqli_fetch_assoc($result);
        $files = [$row["lyrics_file"],$row["lyrics_image"]];

        // Deleting Every File For Sold Lyrics
        foreach($file as $files){
          unlink("./admin/Files/lyrics/$file");
        }

        $query = "DELETE FROM add_lyrics WHERE lyrics_id='$id'";
        $result = mysqli_query($conn,$query);
      }else if($type=="song"){
        ...
        $result = mysqli_query($conn,$query);
        $row = mysqli_fetch_assoc($result);
        $files = [$row["song_file"],$row["song_image"],$row["song_preview"]];

        // Deleting Every File For Sold song
        foreach($file as $files){
          unlink("./admin/Files/song/$file");
        }
        $query = "DELETE FROM add_song WHERE song_id='$id'";
        $result = mysqli_query($conn,$query);
      }
